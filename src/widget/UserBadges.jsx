...
export function UserBadges({badges}) {
    ...
    for(let badge of badges) {
        if(badge==='rookie')
            icons.push(<Tooltip destroyOnHidden key={badge} title="具有新生特别奖资格"><StarTwoTone /></Tooltip>);
        else if(badge==='thanks')
            icons.push(<Tooltip destroyOnHidden key={badge} title="崇高道德的赞许"><LikeTwoTone twoToneColor="#ff0000" /></Tooltip>);
        else if(badge.startsWith('remark:'))
            icons.push(<Tooltip destroyOnHidden key={badge} title={badge.substring(7)}><InfoCircleTwoTone twoToneColor="#ff6600" /></Tooltip>);
        else
